...
export default class extends Controller {
  static targets = ["button", "deleteButton", "loading"];
  ...
  handleSubmitEnd(event, button) {
    ...
    if (event.detail.success !== false) {
      ...
      // The page will refresh/update via Turbo, so we don't need to do much
      setTimeout(() => {
  ...
  handleSubmitError(event, button) {
    ...
    console.error("Status update failed:", event.detail);
  }
  ...
  showSuccess(button) {
    ...
    button.innerHTML = `
      ...
      <span class="hidden sm:inline ml-1.5 text-xs font-medium">Updated!</span>
    `;
  ...
  confirmDelete(event) {
    const button = event.currentTarget;
    const paintName = button.dataset.paintName || "this paint";

    // Custom confirmation dialog
    if (
      !confirm(
        `Are you sure you want to remove ${paintName} from your collection? This action cannot be undone.`,
      )
    ) {
  ...
  handleKeydown(event) {
    ...
    if (event.metaKey || event.ctrlKey) {
      switch (event.key) {
        case "1":
          event.preventDefault();
          this.triggerStatusChange("owned");
          break;
        case "2":
          event.preventDefault();
          this.triggerStatusChange("wishlist");
          break;
        case "3":
          event.preventDefault();
          this.triggerStatusChange("avoid");
          break;
      }
    }
  }
